...
func A_Day_For_Fresh_Sushi() (s Script) {
	...
	s.The("story",
		...
		HasText("author", T("Emily Short")),
		HasText("headline", T("Your basic surreal gay fish romance")),
		Is("scored"),
	)
	//
	s.The("actor",
...
You shake your head. No time for fantasy. Must feed fish.`),
	...
	s.The("studio",
		When("reporting smell").Always(
			g.Say(`The evil fish notices you sniffing the air. "Vanilla Raspberry Roast," it remarks. "You really miss her, don't you."`),
	...
	s.The("container",
		...
		HasText("brief", T("A huge cabinet, in the guise of an armoire, stands between the windows.")),
		HasText("description", T("Large, and with a bit of an Art Nouveau theme going on in the shape of the doors.")),
	)
	// [24-25):
	...
	s.The("prop", Called("paints"),
		HasText("description", T("A bunch of tubes of oil paint, most of them in some state of grunginess, some with the tops twisted partway off.")),
	...
	s.The("prop", Called("cloths"),
		HasText("description", T("Various colors of drapery that Britney uses to set up backgrounds and clothe her models. She does a lot of portraiture, so this comes in handy. It's all a big messy wad at the moment. Organized is not her middle name.")),
	...
	s.The("cloths",
		When("reporting search").
			...
			Always(
				Choose{
					...
					False: g.Go(
						g.The("fish food").IsNow("found"),
						Give("fish food").To("the player"),
		...
		When("reporting shown").Always(
			Choose{
				If: g.The("evil fish").Equals(g.The("action.Context")),
				True: g.Go(
					g.Say("What are you, some kind of sadist? I don't want to see a bunch of cloths! What kind of f'ing good, 'scuse my French, is that supposed to do me? I don't even wear pants for God's sake!"),
	...
	s.The("canister", Called("fish food"),
		HasText("description", T("A vehemently orange canister of fish food.")),
	...
	// [63-56]
	s.The("fish food",
		When("reporting gave").Always(
			Choose{
				If: g.The("evil fish").Equals(g.The("action.Context")),
				True: g.Go(
					g.Say("That's the ticket, sweetie! Bring it on."),
					g.StopHere(),
				),
			}))
	// [69-75)
	s.The("fish food",
		When("being opened by").Always(
			g.Say(`"Oh, for--!" The evil fish breaks out in exasperation and hives. "Screw the screwing around with the screwtop. SHE never has to do that."`),
			g.Say(`"Well, SHE is not here," you reply. "What do you suggest?"`),
			g.Say(`">FEED FISH<" says the fish promptly, making fishy faces and pointing at you with his fin. "Simplicity. Try it."`),
			g.StopHere(),
		),
	)
	// [76-78)
	s.The("fish food",
		When("being inserted").Always(
			g.Say(`"HelLLLOOO," screams the fish. "Whatever happened to FEEDING MEEE?"`),
			g.StopHere(),
		),
	)
	// [79a]
	...
	//[81-88)
	s.The("prop", Called("painting"), HasText("description",
		T(`Only partway finished, but you can tell what it is: Britney's mother. You only met the old woman once, before she faded out of existence in a little hospice in Salzburg.
...
`)))
	...
	s.The("painting",
		After("being examined").Always(
			g.Say(`"So what's it of?" asks the fish, as you turn away. "She never asks if I want to see them, you know?"`),
			g.Say(`"Her mother," you respond without thinking.`),
			g.Say(`"Yeah? Man. I never knew my mother. Eggs, that's the way to go."`),
		))
	s.The("opener", Called("window"), Is("scenery"), In("the studio"))
	s.The("window", Is("hinged").And("closed"))
	s.The("window", When("printing details").Always(
		Choose{
	...
	// [113-114)
	s.The("table", HasText("description", T("A monstrosity of poor taste and bad design: made of some heavy, French-empire sort of wood, with a single pillar for a central leg, carved in the image of Poseidon surrounded by nymphs. It's all scaley, and whenever you sit down, the trident has a tendency to stab you in the knee. But Britney assures you it's worth a fortune.")))
	// [115-116)
	s.The("vase", HasText("description", T("A huge vase -- what you saw once described in a Regency romance as an epergne, maybe -- something so big that it would block someone sitting at the table from seeing anyone else also sitting at the table. But it does function nicely as a receptacle for hugeass bouquets of flowers.")))
	// [117-
	s.The("table",
		When("reporting look under").Always(
			Choose{
	...
	s.The("table",
		After("being examined").Always(
			g.Say(`"That there is MY PA," says the fish, pointing at the scaley triton figure with one fin.`),
		))
	//[123-132)
	s.The("vase",
		When("receiving insertion").Always(
			Choose{
				If: IsNot{g.The("action.Target").Equals(g.The("bouquet"))},
				True: g.Go(
	...
	s.The("player", Possesses("telegram", "bouquet", "lingerie bag"), Wears("chef hat"))
	// 135
	s.The("telegram", HasText("description", T("A telegram, apparently. And dated three days ago. TRIUMPH OURS STOP BACK SOON STOP BE SURE TO FEED FISH STOP")))
	...
	// [150-151);
	s.The("chef hat", HasText("description", T("A big white chef hat of the kind worn by chefs. In this case, you. Just goes to show what a hurry you were in on the way out of the restaurant.")))
	...
	s.The("aquarium", Is("hingeless"),
		HasText("brief", T("In one corner of the room, a large aquarium bubbles in menacing fashion.")),
		HasText("description", T("A very roomy aquarium, large enough to hold quite a variety of colorful sealife -- if any yet survived.")),
		IsKnownAs("tank"),
	)
	s.The("aquarium", Contains("gravel"), Contains("seaweed"))
	// 154a
	s.The("prop", Called("gravel"),
		HasText("description", T("A lot of very small grey rocks.")))
	s.The("gravel", IsKnownAs("little rocks"), Is("plural-named"))
	// 154b
	s.The("prop", Called("seaweed"),
		HasText("description", T("Fake plastic seaweed of the kind generally bought in stores for exactly this purpose.")))
	...
	s.The("animal", Called("evil fish"),
		...
		IsKnownAs("fish"),
		HasText("description", T("Even if you had had no prior experience with him, you would be able to see at a glance that this is an evil fish. From his sharkish nose to his razor fins, every inch of his compact body exudes hatred and danger.")),
	)
	s.The("aquarium", Contains("evil fish"))
	s.The("evil fish", When("reporting take").Always(
		g.Say("The fish swims adroitly out of range of your bare hand."),
		g.Say(`"Hey," he says, and the bubbles of his breath brush against your fingers. "Count yourself lucky I don't bite you right now, you stinking mammal."`),
		g.StopHere(),
	))
	s.The("evil fish", When("reporting attack").Always(
		g.Say("Oh, it's tempting. But it would get you in a world of hurt later on."),
		g.StopHere(),
	))
	s.The("evil fish", When("reporting kiss").Always(
	...
	s.The("lingerie bag", HasText("description", T("You grant yourself the satisfaction of a little peek inside. You went with a pale, silky ivory this time -- it has that kind of sophisticated innocence, and it goes well with the purple of your skin. A small smirk of anticipation crosses your lips.")))
	// 204-
	s.The("lingerie bag",
		After("being examined").Always(
			Choose{
	...
	s.Understand("feed {{something}}").As("feed it")
	s.The("actors",
		// 218:
		Before("feeding it").Always(
			Choose{
				If: g.The("player").Equals(Carrier(g.The("fish food"))),
				False: g.Go(
		...
		// 222-227
		When("feeding it").Always(
			Choose{
				If: g.The("action.Target").Equals(g.The("evil fish")),
				True: g.Go(
					Using{
...
Don't you just hate days where you wake up the wrong color?`),
	...
	s.The("fact", Called("lookedUnderCabinet"), Exists())
	s.The("fact", Called("openedCabinet"), Exists())
	s.The("fact", Called("closedCabinet"), Exists())
	s.The("fact", Called("tookPaints"), Exists())
	s.The("fact", Called("examinedPaints"), Exists())
	s.The("fact", Called("examinedCloths"), Exists())
	s.The("fact", Called("examinedPainting"), Exists())
	s.The("fact", Called("openedWindow"), Exists())
	s.The("fact", Called("lookedUnderTable"), Exists())
	s.The("fact", Called("insertedFlowers"), Exists())
	s.The("fact", Called("examinedTelegraph"), Exists())
	s.The("fact", Called("examinedGravel"), Exists())
	s.The("fact", Called("examinedSeaweed"), Exists())
	s.The("fact", Called("examinedFishOnce"), Exists())
	s.The("fact", Called("examinedFishTwice"), Exists())
	s.The("fact", Called("examinedBouquet"), Exists())
	s.The("fact", Called("smelledBouquet"), Exists())
	s.The("fact", Called("examinedBagOnce"), Exists())
	s.The("fact", Called("examinedBagTwice"), Exists())
	return
}
